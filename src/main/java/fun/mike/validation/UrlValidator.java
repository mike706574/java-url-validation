...
public class UrlValidator implements Serializable {
     ...
     * @return a validation
     */
    public UrlValidation validate(String value) {
        if (value == null) {
            return UrlValidation.invalid(value, "Value is null.");
        }
        ...
        if ("file".equals(scheme)) {// Special case - file: allows an empty authority
            if (authority != null) {
                if (authority.contains(":")) { // but cannot allow trailing :
                    String problem = String.format("Unexpected trailing \":\" in \"file\" URL authority \"%s\".",
                                                   authority);
            ...
            if (!isValidAuthority(authority)) {
                String problem = String.format("Invalid authority \"%s\".",
                                               authority);
                return UrlValidation.invalid(value, problem);
            }
        }
        ...
        if (!isValidPath(path)) {
            String problem = String.format("Invalid path \"%s\".",
                                           path);
            return UrlValidation.invalid(value, problem);
        }
        ...
        if (!isValidQuery(query)) {
            String problem = String.format("Invalid query \"%s\".",
                                           path);
            return UrlValidation.invalid(value, problem);
        }
        ...
        if (!isValidFragment(urlMatcher.group(PARSE_URL_FRAGMENT))) {
            String problem = String.format("Invalid fragment \"%s\".",
                                           path);
            return UrlValidation.invalid(value, problem);
        }
